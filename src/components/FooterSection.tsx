...
export default function FooterSection({ isDark, clientName = 'Medicus', clientLogo = '/images/medicus-logo-white.png', brand = 'standard' }: FooterSectionProps) {
  ...
  const clientLogoSize = clientLogo.includes('logo-aig') ? 'h-6 transition-all' : isZurichLogo ? 'h-8 transition-all' : isPrudentialLogo ? 'h-7 transition-all' : 'h-5 transition-all'
